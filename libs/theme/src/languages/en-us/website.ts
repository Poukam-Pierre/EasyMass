export const website = {
  home: 'Home',
  aboutUs: 'About',
  subtitleAboutUs: 'Welcome to the EasyMesse portal',
  masses: 'Masses',
  faithfull: 'Faithfulls',
  parishes: 'Parishes',
  offerMass: 'Offer Mass',
  massOffer: 'Requests',
  partners: 'Our Partners',
  getInvolved: 'Get Involved',
  getMore: 'Read more',
  footerMessage:
    'EasyMesse is the first platform for requesting masses throughout Cameroon. Its aim is to make life easier for the faithful and the Church.',
  getInTooch: 'Get in tooch',
  heroMessage:
    'Believe in the holy sacrifice, To give thanks to the Lord, Loving our departed loved ones.',
  subtitleHeroMessage:
    "Asking for a Mass to be celebrated for an intention is an act of faith in the power of Christ's sacrifice. It means placing in the hands of Jesus the living and deceased people we love, as well as all our intentions.",
  aboutUsMessage:
    'EasyMesse is a platform that allows you to quickly request a mass across the entire national and international territory. Connected to almost every parish in the country and elsewhere, you can offer a mass of your choice according to your intentions and in the parish of your choice. Trusting us means that you can request a mass from anywhere up to 1 hour before the mass and be sure that your request will be fulfilled.',
  quoteOne: 'Prayer is the elevation of the soul towards God.',
  quoteTwo:
    'Mass is the greatest prayer there is. You may not understand the liturgy, but you know that you are with God.',
  quoteTree:
    "Asking for a Mass means acknowledging that Christ's sacrifice is present and effective for us today.",
  quoteFour:
    'Mass is the high point of all Christian prayer and activity. It is the moment when the soul is most closely united with God.',
  quoteFive:
    'Asking for a mass is an expression of trust in the saving power of Christ and in the intercession of the Virgin Mary and the saints.',
  quoteSix:
    "The Mass is the Church's greatest treasure, because it contains everything we need: Jesus Christ.",
  quoteSeven:
    'Asking for a Mass means offering God the best of what we have: our time, our prayer, our love.',
  quoteEight:
    'Asking for a Mass means entrusting God with a particular intention, entrusting him with those we love and those for whom we pray.',
  quoteNine:
    'Mass is the greatest prayer we can offer for the living and the dead. To ask for a Mass is to offer a treasure of grace to those for whom we pray.',
  quoteTen:
    'Asking for a Mass is a sign of faith in the effectiveness of community prayer. In the unity of the celebration, our individual intentions unite to form a powerful supplication before God.',
  quoteEleven:
    "Asking for Mass is a sign of faith in God's intercession. It means believing that our prayers are heard and that God acts according to his will for the good of all.",
  quoteTwelve:
    "Asking for Mass means joining in Jesus' offering on the altar. It means mystically participating in Christ's redemptive sacrifice for the salvation of the world.",
  quoteThirteen:
    "To ask for a Mass is to recognise our constant need for divine grace. It means humbly placing our intentions in God's hands and trusting him to respond according to his wisdom.",
  quoteFourteen:
    'The request for Mass is an expression of the communion of saints, where the living and the dead are united in prayer and sacrifice for the good of the Church and the world.',
  orderMassHeroMsg:
    'Asking for a mass is an expression of trust in the saving power of Christ...',
  applicantInformation: "Applicant's details",
  massInformations: 'Mass Informations',
  addToList: 'Add to list',
  triduum: 'Triduum',
  seven: 'Seven',
  novena: 'Novena',
  thirty: 'Thirty',
  fullName: 'Full Name',
  city: 'Cities',
  dateTime: 'Date & Time',
  massIntention: 'Mass Intention',
  noProcessMass: 'No request for a mass.',
  phoneNumber: 'Phone Number',
  anonymous: 'Anonymous',
  requestList: 'Request List',
  souscribe: 'Pay Out',
  massRequest: 'Mass Requests',
  billing: 'Total billing',
  applicant: 'Requester',
  amount: 'Amount',
  paymentMethod: 'Payment Method',
  confirmPayment: 'Confirm payment',
  expiredDate: 'Expired date',
  slogan: 'Mass, the greatest of all prayers.',
  greeting: 'Welcome back !',
  logInMsg: 'Enter your login details.',
  password: 'Password',
  newPassword: 'New password',
  confirmPassword: 'Confirm password',
  connexion: 'connection',
  forgotPassword: 'Forgot password ?',
  passwordRecovery: 'Password recovery !',
  fillEmail: 'Enter your e-mail address.',
  send: 'Send',
  fillPassword: 'Enter your new password.',
  save: 'Save',
  numberChecked: 'Must be a number',
  dateTimeChecked: 'Date and time must select',
  intentionChecked: 'Intention must filled in',
  intentionNumberChecked: 'At most 300 words must be',
  finances: 'Finances',
  logout: 'Deconnection',
  withdrawal: 'Withdrawal',
  create: 'Create',
  modify: 'Modify',
  cancel: 'Cancel',
  delete: 'Delete',
  name: 'First name',
  secondName: 'last Name',
  registrationDate: 'Registration time',
  massType: 'Type of masses',
  startDate: 'Start date',
  endDate: 'End date',
  status: 'Status',
  listOfMassSupply: 'List of masses requested',
  listOfMasses: 'List of masses',
  search: 'Search',
  filter: 'Filter',
  year: 'Year',
  month: 'Month',
  week: 'Week',
  addMass: 'Add a mass',
  dayOfMass: 'Mass day',
  massHour: 'Mass time',
  price: 'fee',
  action: 'Actions',
  monday: 'Monday',
  tuesday: 'Tuesday',
  wednesday: 'Wednesday',
  thursday: 'Thursday',
  friday: 'Friday',
  saturday: 'Saturday',
  sunday: ' Sunday',
  day: 'Jours',
  hour: 'Times',
  massModify: 'Modify a mass',
  applyAll: 'Apply to all occurences',
  createMass: 'Create a mass',
  massTypeHolder: 'Enter type of mass',
  massDayHolder: 'Enter day',
  massTimeHolder: 'Enter time',
  massPriceHolder: 'Enter the mass fee',
  duplicateAll: 'Repeat this mass',
  cashRegister: 'Cash register',
  transactionHistory: 'Transactions history',
  transactionId: 'Transaction ID#',
  warningMassCreation: 'Aucune messe créée pour le moment.',
  deleteMass: 'Delete a mass !',
  deleteMassMsg: 'Are you sure you want to abolish this mass ?',
  minPassword: ' At least 5 caracters !',
  passwordCarateristics: '1 uppercase, 1 lowercase et 1 digit',
  comparePassword: 'Non identical password !',
  passwordVisible: 'Get password visible.',
  parishName: 'Enter name',
  parishCity: 'Enter city',
  leadParishName: "Enter the manager's name",
  parishMailAddress: 'Enter email',
  parishPhoneNumber: 'Enter phone number',
  createParish: 'Create parish',
  listOfParish: 'List of parishes',
  addParish: 'Add parish',
  addCity: 'Add city',
  reload: 'Reload',
  parishModify: 'Modify a parish',
  cityModify: 'Modify a city',
  deleleParish: 'Delete parish',
  deleteParishMsg: 'Are you sure you want to abolish this parish ?',
  details: 'Details',
  deleteMassMsgWarning: 'All linked data will be deleted.',
  date: 'Date',
  downloadAll: 'Download All',
  noIntentionMass: 'No request for masses have received yet.',
  dateOfMass: 'Masses Date',
  massTime: 'Masses Time',
  parishInformations: 'Personal informations',
  edit: 'edit',
  massStatistics: 'Statistic of masses',
  period: 'Period',
  massNumber: 'Number of masses',
  massTypeWarningMsg: 'Should choose a mass type',
  dayOfMassWarningMsg: 'Should choose a day',
  massTimeWarningMsg: 'Should choose a time',
  priceWarningMsg: 'Should choose a price',
  footerDescription:
    "EasyMesse puts digital technology at the service of the Church's mission, for an attentive welcome and a simplified organization.",
  usefulLinks: 'Useful Links',
  resources: 'Resources',
  contact: 'Contact',
  faq: 'FAQ',
  blog: 'Blog',
  legalNotice: 'Legal Notice',
  privacyPolicy: 'Privacy Policy',
  allRightsReserved: 'All rights reserved.',
  madeWith: 'Made with',
  aboutHeroTitle: 'Digital technology serving the mission of the Church',
  aboutHeroSubtitle:
    'EasyMesse supports the faithful and parishes with an attentive welcome, simplified organization, and prayers carried all the way to the altar.',
  ourValues: 'Our Values',
  ourValuesSubtitle: 'What guides everything we do, every day.',
  valueFaithTitle: 'Faith',
  valueFaithDescription:
    "Every mass request is carried with the respect and care it deserves, in the spirit of the Church's prayer.",
  valueSimplicityTitle: 'Simplicity',
  valueSimplicityDescription:
    'A clear, intuitive platform designed so offering a mass only takes a few moments.',
  valueTrustTitle: 'Trust',
  valueTrustDescription:
    'We work hand in hand with parishes to make sure every intention is honored.',
  valueAccessibilityTitle: 'Accessibility',
  valueAccessibilityDescription:
    'Request a mass from anywhere, at any time, up to one hour before the celebration.',
  phoneWarningMsg: 'Phone number is required',
  yourInformations: 'Your details',
  paypalRedirectInfo:
    'You will be redirected to a secure PayPal payment page to complete your payment.',
  invoiceEmailHelper: 'Your payment receipt will be sent to this address.',
  checkoutRequiredFields: 'Please enter your name and phone number.',
  checkoutError: 'Something went wrong during payment. Please try again.',
  estimatedBilling: 'Estimated total',
  checkoutReturnTitle: 'Thank you!',
  checkoutReturnDefaultMessage: 'Your payment is being processed.',
  checkoutReturnReceiptEmail:
    'You will receive a confirmation of your purchase and the link to download your receipt by email soon.',
  checkoutReturnReceiptSms:
    'You will receive a confirmation of your purchase and the link to download your receipt by SMS soon.',
  backToHome: 'Back to home',
  email: 'Email',
  emailWarningMsg: 'Email is required',
  invalidEmail: 'Invalid email',
  dashboard: 'Dashboard',
  overview: 'Overview',
  management: 'Management',
  administration: 'Administration',
  administrators: 'Administrators',
  cities: 'Cities',
  priests: 'Priests',
  settings: 'Settings',
  profile: 'Profile',
  role: 'Role',
  authNumber: 'License number',
  createAdministrator: 'Create administrator',
  cannotDeleteSelf: 'You cannot delete your own account.',
  you: 'You',
  saved: 'Saved successfully.',
  processing: 'Processing...',
  loading: 'Loading...',
  noDataYet: 'No data yet.',
  loadErrorGeneric: 'Unable to load data.',
  genericErrorMsg: 'Something went wrong. Please try again.',
  unique: 'Unique',
  selectParish: 'Select a parish',
  selectParishPrompt: 'Please select a parish to continue.',
  selectMass: 'Select a mass',
  selectMassPrompt: 'Please select a mass to continue.',
  estimatedDurationMinutes: 'Estimated duration (minutes)',
  payoutNumber: 'Payout number',
  priestAvailable: 'Available',
  priestUnavailable: 'Unavailable',
  priestsCreateHint:
    'Priests are created from the parish workspace. You can edit or remove a record here.',
  parishBlocked: 'Parish blocked',
  parishUnblocked: 'Parish unblocked',
  parishBlockedTooltip: 'This parish is blocked. Click to unblock.',
  parishActiveTooltip: 'This parish is active. Click to block.',
  payoutBlockToggleTooltip: "Block or unblock this parish's payouts.",
  parishCreatedSuccess: 'Parish created successfully.',
  parishNameWarningMsg: 'Parish name is required',
  parishCityWarningMsg: 'City is required',
  parishPhoneWarningMsg: 'Phone number is required',
  leadParishNameWarningMsg: "Manager's name is required",
  ledgerCorrection: 'Ledger correction',
  ledgerCorrectionHelp:
    "Manually adjust the parish's balance (credit or debit).",
  ledgerCorrectionAmount: 'Amount (positive to credit, negative to debit)',
  ledgerCorrectionNote: 'Reason for the correction',
  recoveryUnavailableTitle: 'Recovery unavailable',
  recoveryUnavailableMessage:
    'Self-service password recovery is not available yet. Please contact a platform administrator.',
  backToLogin: 'Back to login',
  totalParishIncome: 'Total parish income',
  platformFeeRevenue: 'Platform fee revenue',
  totalWithdrawn: 'Total withdrawn',
  totalParishes: 'Total parishes',
  activeParishes: 'Active parishes',
  blockedParishes: 'Blocked parishes',
  revenueByMonth: 'Revenue by month',
  noRevenueData: 'No revenue data available.',
  topParishesByIncome: 'Top parishes by income',
  topParishesByMasses: 'Top parishes by masses',
  recentlyOnboarded: 'Recently onboarded parishes',
  parishesByCity: 'Parishes by city',
  platformFeePercentage: 'Platform fee (%)',
  platformFeeFixedAmount: 'Platform fixed fee',
  platformFeeHelp:
    'This fee is applied to every transaction processed by the platform.',
  viewOnlyField: 'Read only',
  memberSince: 'Member since',
  lastUpdated: 'Last updated',
  massesCreateHint:
    'Masses are created from the parish workspace. You can edit or remove a mass here.',
  createPriest: 'Create priest',
  birthDate: 'Birth date',
  massesCreatedCount: 'Masses created',
  moneyEarned: 'Money earned',
  moneyWithdrawn: 'Money withdrawn',
  intentionsTreatedCount: 'Intentions treated',
  massesByStatus: 'Masses by status',
  massPrices: 'Prices by currency',
  currency: 'Currency',
  withdrawalHint:
    'Set your payout number in your profile first before you can withdraw.',
  payoutNumberHint:
    'This number will be used to receive your withdrawals (mobile money).',
  withdrawalNoPayoutNumber:
    'Set your payout number in your profile before making a withdrawal.',
  withdrawalRecipientFailed:
    "This number couldn't be validated as a mobile money recipient. Check the number in your profile and try again.",
  withdrawalBlocked:
    'Withdrawals are currently blocked for your parish. Contact an administrator.',
  withdrawalInsufficientBalance: 'Insufficient balance to complete this withdrawal.',
  withdrawalNotAccepted:
    'The withdrawal was not accepted by the payment provider. Please try again.',
  withdrawalFailed:
    'Something went wrong with the withdrawal. Please try again.',
  addCurrencyFee: 'Add a currency',
  currencyWarningMsg: 'Currency is required',
  baseCurrencyCannotBeRemoved:
    'The base currency configuration cannot be removed.',
  priceNotAvailableInCurrency:
    "This price isn't available in that currency yet.",
  platformFeeNotConfiguredForCurrency:
    'No platform fee is configured for this currency yet.',
  massPricing: 'Mass Pricing',
  priceBands: 'Price bands',
  priceBandsHelp:
    "A price band gives every mass whose XAF price falls in a range the same price in another currency, so you don't have to set one for each mass individually. A mass with its own price (set on the mass itself) always uses that instead.",
  addPriceBand: 'Add price band',
  editPriceBand: 'Edit price band',
  minPrice: 'Minimum price (XAF)',
  maxPrice: 'Maximum price (XAF)',
  setMassPrice: 'Set price',
  ownPriceHint: "This mass's own price, if set, always overrides any price band for it.",
  transactionType: 'Type',
  allTypes: 'All types',
  filterFrom: 'From',
  filterTo: 'To',
  clearFilters: 'Clear',
  repeatInterval: 'Repeat',
  weekly: 'Weekly',
  monthly: 'Monthly',
  repeatDuration: 'For',
  oneMonth: '1 month',
  threeMonths: '3 months',
  sixMonths: '6 months',
  oneYear: '1 year',
};
