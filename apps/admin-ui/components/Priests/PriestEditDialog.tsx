import { Box, Button, Checkbox, Dialog, FormControlLabel, TextField, Typography } from "@mui/material";
...
interface PriestEditDialogProps {
    isOpen: boolean;
    handleClose: () => void;
...
export default function PriestEditDialog({ isOpen, handleClose, priest, onSaved }: PriestEditDialogProps) {
    const { formatMessage } = useIntl()

    const { handleChange, handleSubmit, errors, touched, values, setFieldValue, isSubmitting } = useFormik({
        initialValues: {
    ...
    return (
        ...
        >
            <Box sx={{ padding: '48px 60px', minWidth: '440px', display: 'grid', rowGap: 2 }} component="form" onSubmit={handleSubmit}>
                <Typography variant="h2" textAlign="center">{formatMessage({ id: 'modify' })}</Typography>
                <TextField
                    name="firstName" placeholder={formatMessage({ id: 'name' })} size="small"
                    value={values.firstName} onChange={handleChange}
                    error={!!(errors.firstName && touched.firstName)}
                />
                <TextField
                    name="secondName" placeholder={formatMessage({ id: 'name' })} size="small"
                    value={values.secondName} onChange={handleChange}
                    error={!!(errors.secondName && touched.secondName)}
                />
                <TextField
                    name="phoneNumber" placeholder={formatMessage({ id: 'phoneNumber' })} size="small"
                    value={values.phoneNumber} onChange={handleChange}
                    error={!!(errors.phoneNumber && touched.phoneNumber)}
                />
                <TextField
                    name="authNumber" placeholder={formatMessage({ id: 'authNumber' })} size="small"
                    value={values.authNumber} onChange={handleChange}
