...
export default function ModalPayment({
  ...
  const [paypalCurrency, setPaypalCurrency] =
    useState<(typeof PAYPAL_CURRENCIES)[number]>('USD');
  ...
  } = usePaymentPreview(massRequested, checkoutCurrency, isOpen);
  const previewError = errorKey ? formatMessage({ id: errorKey }) : '';

  useEffect(() => {
    if (!isOpen) return;
    const lastNamedRequest = [...massRequested]
      .reverse()
      .find((request) => request.faithInfos);
    ...
    [PAYPAL_TAB_INDEX]: (
      <Box sx={{ display: 'grid', rowGap: 1 }}>
        ...
        <TextField
          ...
          onChange={(e) =>
            setPaypalCurrency(e.target.value as typeof paypalCurrency)
          }
        >
  ...
  const handleConfirm = () => {
    ...
    apiMiddleware({
      ...
      data: {
        ...
        massInfos: massRequested.map(
          ({ massInfos: { massId, intention, anonymous } }) => ({
            id: massId,
            intension: intention,
            anonymous: anonymous,
          })
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {paymentMethod.map(
              ({ serviceName, image: { ref, height, width } }, index) => (
                <Tab
                  disableRipple
                  key={index}
                  icon={
        ...
        <Box sx={{ textAlign: 'center' }}>
          ...
          {!isPreviewLoading && !previewError && preview && (
            <Typography
              variant="h5"
              sx={{ paddingBottom: 0, fontWeight: 'bold' }}
            >
              {formatMessage({ id: 'estimatedBilling' })} :{' '}
              {formatNumber(preview.grandTotal, {
                style: 'currency',
        ...
        {errorMessage && (
          <Typography
            variant="body2"
            sx={{ color: 'var(--error)', textAlign: 'center' }}
          >
            {errorMessage}
          </Typography>
        )}
        ...
        >
          {formatMessage({
            id: isSubmitting ? 'processing' : 'confirmPayment',
          })}
        </Button>
      </Box>
    </Dialog>
